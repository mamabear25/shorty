...
app.post('/api/shorten', async (req, res) => {
  ...
  try {
    await qrcode.toFile(qrCodePath, `http://localhost:4001/${id}`); // Generate QR code and save it as a file
  } catch (error) {
    console.error('Error generating QR code:', error);
  }
  ...
  // Associate the shortened URL with the user's identifier
  urlHistory.push({ id, userIdentifier, shortUrl: `http://localhost:4001/${id}`, originalUrl: url });
  urlAnalytics[id] = { clicks: 0, sources: {} };
  ...
  res.json({ shortUrl: `http://localhost:4001/${id}`, qrCode: `http://localhost:4001/qrcodes/${id}.png` });
});
